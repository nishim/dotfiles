...
- 関数が単一責任の原則に従っているか
- エラーメッセージが明確で有用か
- ネストした三項演算子がないか（複数条件は switch または if/else 連鎖を推奨）
- 密なワンライナーより明示的なコードになっているか（短さより読みやすさを優先）
- 過度な抽象化や、構造化に役立つ抽象化の欠如がないか
...
レポートの分量は指摘の実質に合わせること。指摘のない観点のセクションや定型文で水増ししない。
