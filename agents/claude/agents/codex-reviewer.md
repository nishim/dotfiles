...
|---|---|
| 指定なし | `codex -a never -c model_reasoning_effort=xhigh review --uncommitted` |
| `staged` / `unstaged` / `diff` | `codex -a never -c model_reasoning_effort=xhigh review --uncommitted` |
| `branch` または `ブランチ` | `codex -a never -c model_reasoning_effort=xhigh review --base origin/main` |
| `commit <SHA>` | `codex -a never -c model_reasoning_effort=xhigh review --commit <SHA>` |
| その他の自由テキスト | `codex -a never -c model_reasoning_effort=xhigh review "<ユーザーの入力をそのまま>"` |
